...
def animate_2D(data, interval=5, plot_save=False, filename='Animation_grid.gif'):
    """
    Create a 2D animation of a grid from a 3D array.
    ...
    ax.set_title(f'PSM with frequency = {freq_intr} coupling = {coupling}, time delay = {delay}, elongation speed = {elongation_speed}')
...
def animate_2D(data, interval=5, plot_save=False, filename='Animation_grid.gif'):
    ...
    # Convert animation to HTML for Jupyter Notebook
>>>>>>> 148acff67ce5e6f5699323db81a6d7a9acc8a539
    return HTML(ani.to_jshtml())